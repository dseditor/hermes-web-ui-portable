export default {
  ...
  settings: {
    ...
    browser: {
      intro: '把代理的浏览器工具接到本机 Chromium 系浏览器（CDP）。先尝试连接指定端口上已有的浏览器；若没有，就用持久登录 profile 启动一个可见的 Chrome 供你登录。',
      cdpUrl: 'CDP 网址',
      cdpUrlHint: '代理要连接的浏览器调试地址，默认 http://127.0.0.1:9222。',
      chromePath: '浏览器可执行文件路径',
      chromePathHint: '留空＝自动检测系统 Chrome/Edge；填了才覆盖。',
      chromePathPlaceholder: '留空＝自动检测（不变更）',
      launch: '启动并连接',
      launchHint: '连接已有浏览器；没有则用持久登录 profile 启动 Chrome，并写入配置。',
      launchButton: '启动并连接',
      launchStarted: '已触发浏览器（端口 {port}）。若是新启动，请在打开的窗口登录。',
  ...
  platform: {
    ...
    lineChannelAccessTokenHint: 'LINE Developers 后台的长期 channel access token',
    lineChannelSecret: 'Channel Secret',
    lineChannelSecretHint: 'Channel secret（用于 HMAC-SHA256 webhook 验证）',
    linePublicUrl: '公开网址',
    linePublicUrlHint: '对外的 HTTPS 基底网址，用于发送媒体（图片／语音／视频）',
    lineAllowedUsers: '允许的用户',
    lineAllowedUsersHint: '用户 ID 列表，U 开头，以逗号分隔',
    lineAllowedGroups: '允许的群组',
    lineAllowedGroupsHint: '群组 ID 列表，C 开头，以逗号分隔',
    lineAllowedRooms: '允许的聊天室',
    lineAllowedRoomsHint: '聊天室 ID 列表，R 开头，以逗号分隔',
    lineHomeChannel: '主频道',
    lineHomeChannelHint: '定时／通知的发送目标（选填）',
    botId: 'Bot ID',
  ...
  changelog: {
    ...
    new_0_6_7_9: '桌面端下载入口已补充到官网 https://github.com/dseditor/hermes-web-ui-portable，也可以继续从 GitHub Releases 获取最新安装包',
