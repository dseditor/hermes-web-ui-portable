export default {
  ...
  skills: {
    ...
    backTo: '返回',
    attachedFiles: '附件檔案',
    filesTruncated: '（顯示 {shown} / {total} 個，已隱藏打包的 venv／相依套件檔案）',
    loadFailed: '載入技能失敗',
  ...
  pair: {
    ...
    connecting: '配對中，請稍候…',
    codePlaceholder: '輸入配對碼',
    connect: '連線',
    codeRequired: '請輸入配對碼',
    failed: '配對失敗，請確認配對碼是否正確或已過期',
  },
  pairing: {
    title: '對外連線',
    notice: '開啟後會建立一個臨時公開網址，並產生一次性配對碼。用手機掃 QR 或輸入配對碼即可連上，不需通訊軟體、不需手動登入。',
    notConfigured: '此環境未設定對外通道（cloudflared），無法開啟對外連線。',
    loadFailed: '讀取連線狀態失敗',
    idleHint: '目前未對外開放。點擊下方按鈕建立臨時公開網址與配對碼。',
    start: '開啟對外連線',
    started: '對外連線已開啟',
    startFailed: '開啟失敗',
    stop: '停止對外連線',
    ...
    copy: '複製',
    copied: '已複製',
    expiresIn: '配對碼 {time} 後失效',
    expired: '配對碼已失效，請重新產生',
    qrUnavailable: 'QR 產生失敗，請改用配對碼或網址',
  },
  plugins: {
    title: '擴充',
    refresh: '重新整理',
    notice: '依分類管理 Hermes 擴充功能。圖片／影片生成可直接在此選擇提供者並填入金鑰，儲存後會寫入目前設定檔，新的 Hermes 工作階段即生效。（通訊軟體請至「頻道」、模型請至「模型」設定。）',
    providerLabel: '提供者',
    selectProvider: '選擇提供者...',
    providerHint: '選好提供者並填入對應金鑰後按儲存，系統會自動設定，不需要再用 CLI 下指令。',
    save: '儲存',
    saved: '已儲存設定',
    saveFailed: '儲存失敗',
    nothingToSave: '沒有要儲存的內容',
    envSet: '已設定',
    envPlaceholder: '輸入金鑰／設定值',
    envKeepPlaceholder: '已設定，留空則保留原值',
    category: {
      image_gen: '圖片生成',
      video_gen: '影片生成',
      web: '網路搜尋',
      ...
      teams_pipeline: 'Teams 管線',
      other: '其他',
    },
    categoryDesc: {
      image_gen: '選擇圖片生成的提供者並填入金鑰；正確設定後 image_generate 工具就會使用它。',
      video_gen: '選擇影片生成的提供者並填入金鑰；正確設定後 video_generate 工具就會使用它。',
      web: '網路搜尋後端，填入想用的服務金鑰即可啟用。',
      browser: '瀏覽器自動化後端。',
      memory: '長期記憶後端。',
      observability: '可觀測性／追蹤後端。',
    },
    loadFailed: '載入擴充失敗',
    commandCopied: '指令已複製',
  ...
  settings: {
    ...
    browser: {
      intro: '把代理的瀏覽器工具接到本機 Chromium 系瀏覽器（CDP）。先嘗試連線指定埠上既有的瀏覽器；若沒有，就用持久登入 profile 啟動一個可見的 Chrome 供你登入。',
      cdpUrl: 'CDP 網址',
      cdpUrlHint: '代理要連線的瀏覽器除錯位址，預設 http://127.0.0.1:9222。',
      chromePath: '瀏覽器執行檔路徑',
      chromePathHint: '留空＝自動偵測系統 Chrome/Edge；填了才覆寫。',
      chromePathPlaceholder: '留空＝自動偵測（不變更）',
      launch: '啟動並連線',
      launchHint: '連線既有瀏覽器；沒有則用持久登入 profile 啟動 Chrome，並寫入設定。',
      launchButton: '啟動並連線',
      launchStarted: '已觸發瀏覽器（埠 {port}）。若是新啟動，請在開啟的視窗登入。',
      launchFailed: '啟動瀏覽器失敗',
    },
    models: {
      apiKey: 'API Key',
  ...
  platform: {
    ...
    lineChannelAccessTokenHint: 'LINE Developers 後台的長期 channel access token',
    lineChannelSecret: 'Channel Secret',
    lineChannelSecretHint: 'Channel secret（用於 HMAC-SHA256 webhook 驗證）',
    linePublicUrl: '公開網址',
    linePublicUrlHint: '對外的 HTTPS 基底網址，用於傳送媒體（圖片／語音／影片）',
    lineAllowedUsers: '允許的使用者',
    lineAllowedUsersHint: '使用者 ID 清單，U 開頭，以逗號分隔',
    lineAllowedGroups: '允許的群組',
    lineAllowedGroupsHint: '群組 ID 清單，C 開頭，以逗號分隔',
    lineAllowedRooms: '允許的聊天室',
    lineAllowedRoomsHint: '聊天室 ID 清單，R 開頭，以逗號分隔',
    lineHomeChannel: '主頻道',
    lineHomeChannelHint: '排程／通知的發送目標（選填）',
    botId: 'Bot ID',
  ...
  files: {
    ...
    list: '檔案清單',
    breadcrumbRoot: '根目錄',
    setAsWorkspace: '設為工作資料夾',
    workspaceSet: '已設為工作資料夾：{path}',
    workspaceSetFailed: '設定工作資料夾失敗',
    noActiveSession: '請先開啟一個對話',
    newFile: '新增檔案',
    newFolder: '新增資料夾',
    upload: '上傳',
    refresh: '重新整理',
  ...
  changelog: {
    ...
    new_0_6_7_9: '桌面端下載入口已補充到官網 https://github.com/dseditor/hermes-web-ui-portable，也可以繼續從 GitHub Releases 取得最新安裝包',
